        ...
        if (type.equals("transacter")) {
            while (running || stopping) {
                ...
                while (sourceAccount == destinationAccount) {
                    ...
                    sourceAccount = rng.nextInt(length);
                    destinationAccount = rng.nextInt(length);
                    if (sourceAccount == destinationAccount) {
                        destinationAccount = (sourceAccount + 1) % length;
                        break;
                    }
                }
                int amount = 0;
                ...
                if (source.balance > 1) {
                    ...
                    amount = rng.nextInt(source.balance / 2);
                } else {
                    amount = 75;
...
//                    transactionLock.writeLock().unlock();
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            for (int j = 0; j < accountsSize; j++) {
                int balance = 1 + rng.nextInt(5000);
                accountShard2[j] = new Account(balance, 0);
            }

            ShardedBank shardedTotalOrder = new ShardedBank(threads, i, "transacter", size,
                    0,
